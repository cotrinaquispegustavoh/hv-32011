...
def search_staff_view(request):
    ...
    if query: 
        staff = [u for u in staff if query in normalize_text(u.first_name) or query in normalize_text(u.last_name) or query in normalize_text(u.dni)]
    if role_filter: 
        staff = [u for u in staff if u.role == role_filter]
